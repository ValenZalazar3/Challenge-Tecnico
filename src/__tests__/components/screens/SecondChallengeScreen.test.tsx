...
describe("SecondChallengeScreen", () => {
  ...
  it("should allow removing coins", () => {
    ...
    expect(
      screen.queryByDisplayValue(/Tus números son:/i),
  ...
  it("should display the minimum change after submit", () => {
    ...
    fireEvent.change(screen.getByLabelText(/Colocar monedas de cambio:/i), {
      target: { value: "1" },
    });
    fireEvent.click(screen.getByText(/Agregar cambio/i));
    ...
    expect(
      screen.getByDisplayValue("Tu cambio mínimo es: 4 🤑"),
    ).toBeInTheDocument();
  });
});
